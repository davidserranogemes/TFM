        ...
        for (i in seq(from=2, to= length(files.list),by=2)) {
          data <- read.csv(files.list[i], header = FALSE,col.names = c("Epoch", "ACC", "ACC_VAL","OVER"),sep = " ")
          data <- data[,1:3]
          data <- cbind(data,as.numeric(substr(data$Epoch,6,100000)))
          data <- data[,c(4,2,3)] 
          data <- cbind(data,numeric(nrow(data)))
          colnames(data) <- c("Epoch","ACC","ACC_VAL","Model")
          ...
          colnames(data.improve) <- c("Model", "ACC_VAL","MAX_ACC")
